...
response = ""
while response != "q":
    response = input("Please enter food and quantity, in the format: food, quantity\n")
    if ',' in response:
        parts = response.split(',')
        food, quantity = parts[0], int(parts[1])
        inventory[food] = inventory.get(food, 0) + quantity
        for food, quantity in inventory.items():
            print(food, quantity)

#Note: Can also do while True, and in the loop if response is q, break
#After parts, can validate input:
        #Check that there's only two parts
        #Check that one side is alphabetical, other is numerical
