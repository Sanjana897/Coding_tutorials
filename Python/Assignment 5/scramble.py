...
print("I'll ask you for a series of words")
print("When you stop giving words, I'll print out every word with its letters mixed up!")
response = "a"
word_list = []
while response != "":
    response = input("Give me a word, or just press <Return> without typing to stop\n")   
    word_list.append(response)
word_list.pop() #Getting rid of the empty string at the end

wordsy.sort_words_by_length(word_list)
for i, ele in enumerate(word_list):
    print("Word %i: %s - %s" %(i, ele, wordsy.scramble_letters(ele)))
