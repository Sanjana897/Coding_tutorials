...
paragraph = """You bet your ass we're all alike... we've been spoon-fed baby food at
school when we hungered for steak... the bits of meat that you did let slip
through were pre-chewed and tasteless.  We've been dominated by sadists, or
ignored by the apathetic.  The few that had something to teach found us will-
ing pupils, but those few are like drops of water in the desert."""

def translate_passage(paragraph_string):
    translated_passage = ""
    current_word = ""
    for i in range(len(paragraph_string)):
        current_word += paragraph_string[i]
        if paragraph_string[i].isalpha() == False and len(current_word) > 1:
            word_before_this = current_word[:-1]
            translated_passage += pig_latin.to_pig_latin(word_before_this) + paragraph_string[i]
            current_word = ""
        elif paragraph_string[i].isalpha() == False and len(current_word) == 1:
            translated_passage += paragraph_string[i]
            current_word = ""
    return translated_passage

print(translate_passage(paragraph))
